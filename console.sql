...
SELECT count(brand) AS COUNT FROM cars WHERE brand = 'Chevrolet';

-- #7 Display the average year of manufacture of cars
SELECT avg(year_of_issue) as avg FROM cars;
...
SELECT *, LENGTH(brand) as length from cars where LENGTH(brand) = 5;

-- #12 Display all amounts where car's brand is Mercedes-Benz
SELECT count(brand) AS COUNT FROM cars where brand = 'Mercedes-Benz';

-- #13 Display the most expensive car & the most cheep car
SELECT * FROM cars WHERE price = (SELECT max(price) FROM cars) or
                         price = (SELECT min(price) FROM cars) ORDER BY price desc;
